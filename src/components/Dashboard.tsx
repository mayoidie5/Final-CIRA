...
export const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  const QuickActionButton = ({ icon: Icon, label, onClick, color }: any) => (
    <button
      onClick={onClick}
      className={`flex items-center gap-3 p-4 rounded-lg border-2 border-${color}-200 dark:border-${color}-800 bg-${color}-50 dark:bg-${color}-900/20 hover:bg-${color}-100 dark:hover:bg-${color}-900/30 transition-colors`}
    >
      <Icon className={`text-${color}-600 dark:text-${color}-400`} size={24} />
      <span className={`text-${color}-700 dark:text-${color}-300`}>{label}</span>
    </button>
  );
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-6">
      ...
      {/* Quick Actions */}
      <div>
        <h3 className="text-gray-800 dark:text-white mb-4">Quick Actions</h3>
